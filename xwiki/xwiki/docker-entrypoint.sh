...
function configure() {
  ...
  file_env 'DB_USER' 'ehradm'
  file_env 'DB_PASSWORD' 'ehrdev'
  file_env 'DB_POD' 'postgres-0'
  file_env 'DB_SVC' 'postgres-service'
  file_env 'DB_PORT' '5432'
  file_env 'DB_NS' 'huni-comn'
  file_env 'DB_HOST' ''$DB_POD'.'$DB_SVC'.'$DB_NS'.svc.cluster.local'
  ...
  safesed "replacedbport" $DB_PORT /usr/local/tomcat/webapps/$CONTEXT_PATH/WEB-INF/hibernate.cfg.xml
  ...
  xwiki_set_properties 'environment.permanentDirectory' '/usr/local/xwiki'
