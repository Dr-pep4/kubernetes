...
# 기존 JAVA_OPTS에 PostgreSQL 관련 시스템 속성 추가
export JAVA_OPTS="$JAVA_OPTS -Dpostgres.host=$POSTGRES_HOST -Dpostgres.port=$POSTGRES_PORT -Dpostgres.db=$POSTGRES_DB -Dpostgres.user=$POSTGRES_USER -Dpostgres.passwd=$POSTGRES_PASSWORD -Djava.awt.headless=true"

# CATALINA_OPTS 설정
export CATALINA_OPTS="$CATALINA_OPTS -server -Xms2048m -Xmx4096m -Dfile.encoding=utf-8 -Djava.awt.headless=true -XX:+UseG1GC -XX:MaxGCPauseMillis=100"
